...
@section('content')
    <div class="animate-fade-in">
        ...
        <section class="mb-12 text-center">
            ...
            <p class="text-gray-500 text-lg max-w-2xl mx-auto mb-8">Jelajahi ribuan penyedia jasa terpercaya sesuai kebutuhan
                Anda</p>

            <!-- Large Search Bar -->
            <div class="max-w-3xl mx-auto relative">
                <form method="GET" action="{{ route('providers.index') }}"
                    class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden flex items-center p-1 focus-within:ring-2 focus-within:ring-primary/50">
                    <i class="fas fa-search text-gray-400 ml-4"></i>
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari jasa atau penyedia (contoh: les matematika, desain logo)"
                        class="flex-1 px-4 py-4 outline-none text-gray-700 placeholder-gray-400">
                    <button type="submit"
                        class="bg-primary hover:bg-primary/90 text-white font-medium px-6 py-3 rounded-xl transition mr-1">Cari</button>
                </form>
        ...
        <section
            class="sticky top-20 z-40 bg-white/95 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-4 mb-8">
            <div class="flex flex-wrap items-center gap-4 justify-between">
                <div class="flex flex-wrap gap-3">
                    <select name="category"
                        class="filter-select px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-primary">
            ...
            <div class="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-100">
                @foreach ($categories->take(8) as $cat)
                    <a href="{{ route('providers.index', array_merge(request()->query(), ['category' => $cat])) }}"
                        class="px-4 py-1.5 {{ request('category') == $cat ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-600' }} rounded-full text-sm font-medium cursor-pointer hover:bg-primary hover:text-white transition">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($topProviders as $provider)
                        <a href="{{ route('providers.show', $provider->slug) }}"
                            class="group bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border border-gray-100 block">
                            <div class="flex items-start gap-4">
                                <!-- Avatar -->
                                <div
                                    class="w-16 h-16 rounded-full bg-gradient-to-br from-primary/20 to-blue-100 overflow-hidden flex-shrink-0">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center justify-between">
                                        <h3 class="font-bold text-gray-900 group-hover:text-primary transition">
                                            {{ $provider->name }}</h3>
                                        <span
                                            class="bg-yellow-100 text-yellow-700 text-xs font-bold px-2 py-0.5 rounded-full">Top
                                            Rated</span>
                                    </div>
                                    <div class="flex items-center gap-1 mt-1">
                                        <i class="fas fa-star text-yellow-400 text-sm"></i>
                                        <span
                                            class="font-semibold text-sm">{{ number_format($provider->average_rating, 1) }}</span>
                                        <span class="text-gray-400 text-xs">({{ number_format($provider->total_reviews) }}
                                            ulasan)</span>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 mt-2">
                                        <span class="text-xs text-gray-400"><i class="fas fa-briefcase"></i>
                                            {{ $provider->services_count }} jasa</span>
                                        <span class="text-xs text-gray-400"><i class="fas fa-shopping-bag"></i>
            ...
            <section class="mb-16">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    @foreach ($newProviders as $provider)
                        <a href="{{ route('providers.show', $provider->slug) }}"
                            class="group bg-white rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02] border border-gray-100 block">
                            <div class="relative">
                                <div
                                    class="w-16 h-16 rounded-full mx-auto bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                                    @if (!empty($provider->avatar))
                                        <img src="{{ \Illuminate\Support\Str::startsWith($provider->avatar, 'http')
                                            ? $provider->avatar
                                            : asset('storage/' . $provider->avatar) }}"
                            ...
                            <h3
                                class="font-semibold text-gray-800 text-center mt-2 text-sm line-clamp-1 group-hover:text-primary transition">
                                {{ $provider->name }}</h3>
                            <div class="flex justify-center items-center gap-1 mt-1">
                                <i class="fas fa-star text-yellow-400 text-xs"></i>
                                <span
                                    class="text-xs font-semibold">{{ number_format($provider->average_rating, 1) }}</span>
                            </div>
                    ...
                    @foreach ($activeProviders as $provider)
                        <a href="{{ route('providers.show', $provider->slug) }}"
                            ...
                            <div class="flex gap-4">
                                <div
                                    class="w-14 h-14 rounded-full bg-gradient-to-br from-green-100 to-green-200 overflow-hidden flex-shrink-0">
                                    @if (!empty($provider->avatar))
                                        <img src="{{ \Illuminate\Support\Str::startsWith($provider->avatar, 'http')
                                            ? $provider->avatar
                                            : asset('storage/' . $provider->avatar) }}"
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center justify-between">
                                        <h3 class="font-bold text-gray-900 group-hover:text-primary transition">
                                            {{ $provider->name }}</h3>
                                        <span
                                            class="bg-green-100 text-green-700 text-xs font-bold px-2 py-0.5 rounded-full">Aktif</span>
                                    </div>
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span
                                            class="text-gray-500 text-xs">{{ $provider->category ? ucfirst(str_replace('_', ' ', $provider->category)) : 'Penyedia' }}</span>
                                    </div>
                                    <div class="flex items-center gap-3 mt-2">
                                        <span class="text-primary font-bold">{{ $provider->services_count }} jasa</span>
                                        <span class="text-xs text-green-600"><i class="fas fa-clock"></i> Aktif</span>
                                    </div>
                                </div>
                            </div>
                        </a>
        ...
        <section class="mb-16">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-gray-500 mt-1">Menampilkan {{ $providers->total() }} penyedia berdasarkan rating &
                        aktivitas terbaru</p>
                </div>
                <select name="sort"
                    class="sort-select px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-primary">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($providers as $provider)
                    <a href="{{ route('providers.show', $provider->slug) }}"
                        ...
                        <div class="flex gap-4">
                            <div
                                class="w-16 h-16 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden flex-shrink-0">
                                @if (!empty($provider->avatar))
                                    <img src="{{ \Illuminate\Support\Str::startsWith($provider->avatar, 'http')
                                        ? $provider->avatar
                                        : asset('storage/' . $provider->avatar) }}"
                            ...
                            <div class="flex-1">
                                <div class="flex items-center justify-between flex-wrap gap-1">
                                    ...
                                    @if ($provider->average_rating >= 4.8)
                                        <span
                                            class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">Top
                                            Rated</span>
                                    @elseif($provider->status === 'active')
                                        <span
                                            class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-100 text-green-700">Aktif</span>
                                    @endif
                                </div>
                                <div class="flex items-center gap-2 mt-1">
                                    <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-gray-400 text-xs">({{ number_format($provider->total_reviews) }}
                                        ulasan)</span>
                                    <span class="text-gray-300">•</span>
                                    <span class="text-gray-500 text-xs">{{ $provider->services_count }} jasa</span>
                                </div>
                                <p class="text-gray-500 text-xs mt-2 line-clamp-1">
                                    {{ Str::limit($provider->bio ?? 'Penyedia jasa profesional siap membantu kebutuhan Anda', 80) }}
                                </p>
                                <div class="flex items-center gap-3 mt-2">
                                    <span class="text-xs text-gray-400"><i class="fas fa-map-marker-alt"></i>
                ...
                @empty
                    <div class="col-span-3 text-center py-16">
                        ...
                        <h2 class="text-lg font-semibold text-gray-800 mb-2">Belum ada penyedia tersedia</h2>
                        <p class="text-gray-500 text-sm mb-4 max-w-md mx-auto">Saat ini belum ada penyedia jasa yang
                            terdaftar. Silakan cek kembali nanti.</p>
                        <a href="{{ route('providers.index') }}"
                            class="inline-block px-5 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition">Refresh
                            Halaman</a>
                    </div>
                @endforelse
            </div>
