...
@section('content')
    ...
    <section class="py-12 md:py-20 animate-fade-in">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div class="space-y-6 text-center lg:text-left">
                ...
                <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-gray-900 leading-[1.2]">Temukan
                    Jasa Terbaik<br>untuk Kebutuhan Anda</h1>
                <p class="text-gray-500 text-lg max-w-md mx-auto lg:mx-0 leading-relaxed">Dari les privat hingga layanan
                    profesional, VEXORA menghubungkan Anda dengan penyedia jasa terpercaya dengan mudah dan cepat.</p>
                <div class="pt-2"><a href="{{ url('/catalog') }}"
            ...
            <div class="relative flex justify-center lg:justify-end">
                <div class="relative w-full max-w-md">
                    ...
                    <div class="animate-float-slow"><img
                            src="https://images.unsplash.com/photo-1521791136064-7986c2920216?w=600&h=600&fit=crop"
                            alt="Layanan profesional" class="w-full h-auto object-contain drop-shadow-xl rounded-3xl"></div>
                    <div
                        class="absolute bottom-6 left-6 bg-white/90 backdrop-blur-sm rounded-2xl px-4 py-2 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-2"><i class="fas fa-check-circle text-primary text-sm"></i><span
                                class="text-gray-800 font-medium text-sm">10.000+ Penyedia</span></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-12">
        <div
            ...
            <div class="flex-1 text-center md:text-left space-y-4">
                <h3 class="text-2xl md:text-3xl font-bold text-gray-900">Temukan Jasa Berkualitas<br>dengan Harga Terjangkau
                </h3>
                <p class="text-gray-600 max-w-sm mx-auto md:mx-0">Dapatkan penawaran terbaik dari penyedia jasa terpercaya
    ...
    <section class="py-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach ($services as $service)
                <a href="{{ route('catalog.show', $service->slug) }}"
                    class="group bg-white rounded-2xl hover:shadow-xl transition-all duration-300 hover-lift border border-gray-100 overflow-hidden block">
                    <div class="card-image-zoom"><img
                            src="{{ Str::startsWith($service->image, 'http') ? $service->image : asset('storage/' . $service->image) }}"
                            alt="{{ $service->title }}" class="w-full h-48 object-cover"></div>
                    <div class="p-5">
                        <h3 class="font-bold text-gray-900 text-lg">{{ $service->title }}</h3>
                        <p class="text-gray-500 text-sm">{{ optional($service->provider)->name }}</p>
                        <div class="flex items-center gap-1 mt-2"><i class="fas fa-star text-yellow-400 text-sm"></i><span
                                class="font-semibold">{{ $service->rating }}</span><span
                                class="text-xs text-gray-500">{{ $service->total_orders }}</span></div>
                        <div class="mt-3"><span class="text-xl font-bold text-primary">Rp
                                {{ number_format($service->price, 0, ',', '.') }}</span><span
                                class="text-xs text-gray-400 ml-1">/ sesi</span></div>
                        <div
                            class="mt-4 block text-center text-primary text-sm font-medium border border-primary/30 rounded-full py-2 hover:bg-primary hover:text-white transition">
                            Pesan Layanan
                        </div>
                    </div>
                </a>
        ...
        <div class="flex justify-center mt-14"><a href="{{ url('/catalog') }}"
                class="px-8 py-3 rounded-full border border-gray-300 text-gray-700 text-sm font-medium hover:border-primary hover:text-primary transition">Cari
                Jasa Lainnya</a></div>
    </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @php
                $testimonials = [
                    [
                        'name' => 'Anita Wijaya',
                        'feedback' => 'VEXORA sangat membantu saya menemukan guru les dengan cepat dan terpercaya.',
                        'rating' => 5,
                        'avatar' => 'https://randomuser.me/api/portraits/women/44.jpg',
                    ],
                    [
                        'name' => 'Budi Santoso',
                        'feedback' => 'Saya cari tukang perbaikan AC di sini, dalam 1 jam langsung datang.',
                        'rating' => 5,
                        'avatar' => 'https://randomuser.me/api/portraits/men/32.jpg',
                    ],
                    [
                        'name' => 'Citra Lestari',
                        'feedback' => 'Desain logo saya dikerjakan dengan sangat baik. Penyedia jasa responsif.',
                        'rating' => 4,
            ...
            @foreach ($steps as $step)
                <div class="text-center group">
                    <div class="relative inline-block mb-6">
                        <div
                            class="w-24 h-24 bg-secondary rounded-2xl flex items-center justify-center mx-auto group-hover:bg-primary/10 transition">
    ...
    <section class="py-16">
        <div class="bg-primary rounded-3xl p-12 md:p-16 text-center text-white shadow-xl">
            ...
            <p class="text-blue-100 text-lg max-w-xl mx-auto mb-8">Mulai sekarang dan temukan layanan terpercaya hanya di
                VEXORA.</p>
